...
「Magic Sort!」風の色分けソートパズル。ボトルに入り混じった色付きのポーションを注ぎ分け、各ボトルを単色にそろえるとクリア——完成した瞬間に**はじけるポップ演出**が弾けます。依存ライブラリなしのバニラ HTML / CSS / JS で動作します。

> 旧名は「Water Puzzle」。同名タイトルとの重複を避けて **PotionPop** に改名しました（リポジトリ名は GitHub の Settings → Rename からご自身で変更できます）。
...
- **完成/クリア演出（PotionPop）**：ボトルが単色で満タンになると、放射フラッシュ＋二重の衝撃リング＋弾け飛ぶスパークル＋**はじけるポップ・バブル**＋スクワッシュ・バウンドで祝福。クリア時は全ボトルが連鎖ポップ→紙吹雪＋**スター評価（最短手数と比較して★1〜3）**＋獲得コイン表示。
